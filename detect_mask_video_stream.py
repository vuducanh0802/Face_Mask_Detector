# How to use
# python detect_mask_video_stream.py
    ...
    try:
        ...
        for (box, pred) in zip(locs, preds):
            ...
            if str(type(detector.where(frame))) == "<class 'tuple'>":
                if endX > float(detector.where(frame)[0])*frame.shape[1] > startX and endY > float(detector.where(frame)[1])*frame.shape[0] > startY:
                    label = "Hand Down, No Mask Detected"
                    color = (0, 0, 255)
                    prob = 0
